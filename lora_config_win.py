...
class LoraConfig(object):
  ...
  def config(self, scrrenid, paramter_list):
    ...
    win = tk.Toplevel(scrrenid)
    win.geometry(size_str)
    win.resizable(width=False, height=False)
    #设置右上角的X功能
    win.protocol("WM_DELETE_WINDOW", lambda arg=win: self.FuncButtonCancel(arg))
    #设置窗口始终在最上层
    win.wm_attributes("-topmost", 1)
    win.title("Lora Config")
    ...
    self.baud_label = tk.Label(win, text='波特率')
    self.baud_combox = ttk.Combobox(win, values=combox_baud)
    self.baud_combox.bind("<<ComboboxSelected>>", self.select_baud)
    
    self.parity_label = tk.Label(win, text='校验位')
    self.parity_combox = ttk.Combobox(win, values=combox_parity)
    self.parity_combox.bind("<<ComboboxSelected>>", self.select_parity)
    
    self.speed_label = tk.Label(win, text='空中速率')
    self.speed_combox = ttk.Combobox(win, values=combox_speed)
    self.speed_combox.bind("<<ComboboxSelected>>", self.select_speed)
    
    self.senddb_label = tk.Label(win, text='发射功率')
    self.senddb_combox = ttk.Combobox(win, values=combox_senddb)
    self.senddb_combox.bind("<<ComboboxSelected>>", self.select_senddb)
    
    self.crc_label = tk.Label(win, text='前向纠错')
    self.crc_combox = ttk.Combobox(win, values=combox_crc)
    self.crc_combox.bind("<<ComboboxSelected>>", self.select_crc)
    
    self.sendmode_label = tk.Label(win, text='传输方式')
    self.sendmode_combox = ttk.Combobox(win, values=combox_sendmode)
    self.sendmode_combox.bind("<<ComboboxSelected>>", self.select_sendmode)
    
    self.waketime_label = tk.Label(win, text='唤醒时间')
    self.waketime_combox = ttk.Combobox(win, values=combox_wakeuptime)
    self.waketime_combox.bind("<<ComboboxSelected>>", self.select_wakeuptime)
    
    self.iomode_label = tk.Label(win, text='IO驱动')
    self.iomode_combox = ttk.Combobox(win, values=combox_iomode)
    self.iomode_combox.bind("<<ComboboxSelected>>", self.select_iomode)
    
    test_cmd = win.register(self.test_input_is_digit)
    self.addr_label = tk.Label(win, text='地址')
    self.addr_string = tk.StringVar()
    self.addr_entry = tk.Entry(win, textvariable=self.addr_string, validate='key', validatecommand=(test_cmd, '%P'))
    
    self.channel_label = tk.Label(win, text='信道')
    self.channel_string = tk.StringVar()
    self.channel_entry = tk.Entry(win, textvariable=self.channel_string, validate='key', validatecommand=(test_cmd, '%P'))
    
    self.writebutton = tk.Button(win, command=self.func_write, text='写入参数')
    self.readbutton = tk.Button(win, command=self.func_read, text='读取参数')
